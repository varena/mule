...
function getUploadAccess() {
  ...
  $admin = $uploader->admin;
  if($admin) {
    return Attachment::$ACCESS_NAMES; // allow all access values
  }
  if($author) {
    return array_intersect_key(Attachment::$ACCESS_NAMES,
                               array(Attachment::ACCESS_PUBLIC => true, Attachment::ACCESS_AUTHOR => true));
  }
  return array();
